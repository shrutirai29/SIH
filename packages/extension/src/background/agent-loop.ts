...
export class AgentLoop {
  ...
  interrupt(reason = 'Page navigated away — task stopped.'): void {
    ...
    this.#abort = null;
    this.transmissions.clear();
    this.#patch({ phase: 'interrupted', message: reason });
  }
  ...
  return;
      ...
      const actRes = await this.#actAll(actions, signal);
      if (signal.aborted) return;
      ...
      const askAction = actions.find((a) => a.op === 'ask_user');
      if (askAction && askAction.op === 'ask_user') {
        this.#patch({
          phase: 'idle',
          message: '💬 PRAHARI Question: ' + (askAction.question || 'I need more information to proceed.'),
        });
        return;
      }
  ...
  async #actAll(actions: readonly Action[], signal: AbortSignal): Promise<{ outcome: ActionResult['outcome']; detail?: string | undefined }> {
    let last: ActionResult['outcome'] = 'no_change';
    let lastDetail: string | undefined;
    for (const action of actions) {
      if (signal.aborted) return { outcome: 'blocked' };
      if (action.op === 'done' || action.op === 'fail' || action.op === 'ask_user') return { outcome: 'advanced' };
